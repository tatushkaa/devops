       ...
       */
         ...
         */
           ...
           */
			...
			GameView.prototype.drawPlayer = function(ctx, playerId, police, x, y, cellSize) {
				...
				const img = police ? (self ? $imgPoliceSelf : $imgPolice) : self ? $imgThiefSelf : $imgThief;
				ctx.save();
         ...
         */
				utils.reWriteDomElement(
					...
					`<div class='game-caption-name'>${name} <span class='game-caption-status game-caption-status-${status}'>${utils.getStatusName(
						status
					)}</span></div>`
				);
			};
			GameView.prototype.setTimer = function(data) {
				...
				let minutes = data.m;
				const timerState =
					minutes > 0 || seconds > 30 ? 'game-timer-ok' : seconds > 15 ? 'game-timer-warn' : 'game-timer-cri';
				if (seconds < 10) {
					seconds = '0' + seconds;
				}
				if (minutes < 10) {
         ...
         */
				utils.reWriteDomElement(
                    ...
                    <span class='game-team-role game-team-role-${role}'>${ROLE_TITLES[team.role] ||
						ROLE_TITLES[team.role]}</span>
                    </div>`
				);
			};
         ...
         */
				// ...
				// нужно скрыть $error и $gameContainer и показать $loading
